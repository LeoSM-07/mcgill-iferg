...
export const publicationType = defineType({
  ...
  fields: [
    ...
    defineField({
      name: "url",
      title: "Published Article URL",
      description: "Use this if the article PDF is not publicly available.",
      type: "url",
    }),
  ],
  validation: (Rule) =>
    Rule.custom((fields) => {
      if (!fields?.pdfFile && !fields?.url) {
        return "You must either upload a PDF or provide a URL to the published article.";
      }
      return true;
    }),
  preview: {
